import Image from "next/image";

const MainPlayer = ({ currentBeat, isPlaying, toggleAudio }) => {
  return (
    <div className="fixed bottom-0 container backdrop-blur bg-opacity-50 rounded-xl">
      <div className="flex p-[5px]">
        <div className="flex w-1/3 justify-center items-center ">
        <img src={currentBeat.photoUrl} alt="beat cover" className="rounded-lg" width={65} height={65}/>
          <div className="flex flex-col px-[16px]">
            <span className="font-medium text-[20px]">
              {currentBeat.title}
            </span>
            <span className=" text-[18px] text-[#9e9b9b]">talllorenc</span>
          </div>
        </div>

        <div className="flex w-1/3 justify-center items-center gap-[25px]">
          <Image
            src="/header/like.png"
            width={20}
            height={20}
            alt="prev img"
          />
          <button onClick={() => toggleAudio(currentBeat.audio)}>
          <Image
              src={isPlaying ? "/header/pause-button.png" : "/header/play-button.png"}
              width={30}
              height={30}
              alt={isPlaying ? "pause img" : "play img"}
            />
          </button > 
          <Image
            src="/header/sound.png"
            width={20}
            height={20}
            alt="next img"
          />
        </div>

        <div className="flex w-1/3 justify-center items-center gap-[10px]">
          <button className="border-[2px]  p-2 rounded-lg hover:bg-red-600">
          <Image
            src="/header/download.png"
            width={20}
            height={20}
            alt="next img"
          />
          </button>
          <button className="border-[2px] p-2 rounded-lg hover:bg-blue-600">
          <Image
            src="/header/share.png"
            width={20}
            height={20}
            alt="next img"
          />
          </button>
        </div>
      </div>
    </div>
  );
};

export default MainPlayer;
