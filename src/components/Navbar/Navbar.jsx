      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-[15px] items-center">
          <div className="flex gap-[20px] items-center">
            {links.map((link) => (
              <Link key={link.id} href={link.url} className={`hover:text-[#9C9C9C] ${pathname === link.url ? 'border-b-2 border-[#F75380] text-[#9C9C9C]' : ''}`}>
                {link.title}
              </Link>
            ))}
          </div>
          <div>
            <button className="flex items-center">
              <Image src="/navbar/cart.png" width={35} height={35} alt="cart icon" />
              <span>$0.00</span>
            </button>
          </div>
